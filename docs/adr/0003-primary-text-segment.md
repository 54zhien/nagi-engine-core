...
- **`text(inUTF16:)` 的完整语义**：
  ...
  - **合法的空区间返回 `""`**；
  - **任一条件不成立返回 `nil`**。
  - **`lowerBound <= upperBound` 是 `Range` 自身的不变量，本方法不重复承担它。** 违反该前置条件的值，意味着**调用方已经违反了 `Range` 的契约** —— 那个值**不在 `PrimaryTextSegment` 的支持域**。本层不替上游类型重复它的契约。
    - **某个构建配置是否动态执行并 trap，属于标准库与优化配置的细节。** 本次 Swift 6.1.2 **Debug** CI（run `34921959000`）确实**在进入本方法之前**就 trap（`Swift/Range.swift:179`，进程级 SIGTRAP，不是断言失败），但那只作为**该配置**的证据，**不是**对「任何配置下都必然如此」的断言。
    - 由此：guard **不恢复**。此前那段「调用方可以造出逆序 `Range`、所以方法必须自己拒绝」的判断作废；纠正后的口径只到「**不重复上游类型契约**」为止。
