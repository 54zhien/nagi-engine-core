...
**R0 的定案**（全文见 ADR-0003）：`PrimaryTextSegment` 是**一个完整 `DocumentUnit` 的 unit-local primary text**，不是 publication-global 文本、也不是 `ContentShard`；构造器**不解析、不折叠、不归一化**，canonical 化由 ingest 负责；**不带 `Codable` / `Hashable` / `NodeID` / 元素树 / 样式 / ruby annotation / 几何**；合法 storage boundary 是 `0...utf16Count` 中不切开 surrogate pair 的位置（负数、越界、`Int.max` 一律 `false`）；`text(inUTF16:)` 的参数是**半开区间**，**两端必须均为合法 storage boundary**、**非负与上界由方法自己检查**（且在任何索引或切片运算之前），成功时返回该区间的精确文本，**合法的空区间返回 `""`**，任一条件不成立返回 `nil`；**有序性不由方法重复承担** —— `lowerBound <= upperBound` 是 `Range` 自身的不变量；违反它的值意味着调用方**已经违反 `Range` 的契约**，**不在本 API 支持域**，本层不替上游类型重复其契约（某个构建配置是否动态执行并 trap 属标准库/优化配置细节；本次 Swift 6.1.2 **Debug** CI run `34921959000` 确实在进方法前 trap，仅作**该配置**的证据）；且**不得把半个 surrogate 静默解码成 U+FFFD**。
